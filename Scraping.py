...
# homesからのスクレイピング（複数の地域）
base_url_template = "https://www.homes.co.jp/chintai/tokyo/{}/list/?page={}"
districts = ["minato-city", "chiyoda-city", "chuo-city", "shinjuku-city", "shibuya-city", "meguro-city"]
headers = {
...
for district in districts:
    for page in range(1, max_page + 1):
        ...
        print("District", district, "Page", page, "Items", len(items))
...
# スーモからの情報取得
# 基本URLテンプレートと地域のリスト(間取り画像がなぜか取れない・・・)
base_url_template = "https://suumo.jp/chintai/tokyo/{}/?page={}"
districts = ["sc_minato", "sc_chiyoda", "sc_chuo", "sc_shinjuku", "sc_shibuya", "sc_meguro"]
max_page = 5  # 最大ページ数
...
for district in districts:
    for page in range(1, max_page + 1):
        ...
        print(f"{district} - page {page} - items {len(items)}")
        ...
        for item in items:
            ...
            for tbody in tbodys:
                ...
                property_image_element = item.find("div", class_="cassetteitem_object-item")
...
def get_geocode_from_name(address):
    address = address.replace(" ", "+")  # 空白をプラスに置き換える
...
df['緯度'], df['経度'] = zip(*df['アドレス'].apply(get_geocode_from_name))
